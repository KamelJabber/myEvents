...
admin.initializeApp();
const onCreate = (collection, snapshot, context) => {
...
exports.onCreateAddress = functions
    .firestore
    .document('addresses/{docId}')
    .onCreate((snapshot, context) => {
    return onCreate('addresses', snapshot, context);
});
exports.onCreateReduceAddress = functions
    .firestore
        ...
        if (snaps.empty || snaps.docs.length === 0) {
            ...
            if (hasPrimary && address.primary === true) {
                snaps.docs.forEach(snapDoc => {
                    if (snapDoc.id !== id) {
                        promises.push(db.collection('addresses')
                            .doc(snapDoc.id)
